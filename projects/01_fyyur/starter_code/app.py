...
def search_venues():
    searchTerm = request.form.get('serach_term', '')
    results = Venue.query.filter(Venue.name.ilike('%'+searchTerm+'%'))
    return render_template('pages/search_venues.html', results=results, search_term=searchTerm)
...
def show_venue(venue_id):
  # shows the venue page with the given venue_id
    venueId = Venue.query.filter(id==venue_id).first()
    return render_template('pages/show_venue.html', venue=venueId)
...
def create_venue_submission():
  ...
  # TODO: modify data to be the data object returned from db insertion
    ...
    try: 
        venue = Venue(name=form.name.data, city=form.city.data, state=form.state.data, phone=form.phone.data, address=form.address.data, genres=genres, seeking_talent=form.seeking_talent.data,
        seeking_description=form.seeking_description.data, image_link=form.image_link.data, website=form.website.data, facebook_link=form.facebook_link.data)
...
def delete_venue(venue_id):
  ...
  # clicking that button delete it from the db then redirect the user to the homepage
    try: 
      venueId = Venue.query.filter_by(id=venue_id).firs()
...
def edit_venue_submission(venue_id):
  ...
  # venue record with ID <venue_id> using the new attributes
    ...
    if error:
      ...
      return redirect(url_for('show_venue', id=venue_id))
  ...
  # TODO: modify data to be the data object returned from db insertion
    ...
    try:
        artist = Artist(name=form.name.data, city=form.city.data, state=form.state.data, phone=form.phone.data, genres=genres, 
                        seeking_venue=False, seeking_description=form.seeking_description.data, image_link=form.image_link.data, facebook_link=form.facebook_link.data)
    ...
    if error:
        ...
        return redirect(url_for('show_artist', id=artistId))
  # on successful db insert, flash success
  # TODO: on unsuccessful db insert, flash an error instead.
...
def create_show_submission():
    ...
    artistShows= db.session.query(Artist, Venue, Show).join(Show, Show.artist_id == Artist.id).join(Venue, Venue.id==Show.venue_id)
...
def search_shows():
  ...
  # on sucessful db insert, flash success
  # flash('Show was successfully listed!')
  # TODO: on unsuccessful db insert, flash an error instead.
